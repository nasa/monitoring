...
def plan_to_req(
    plan: PositionReportsPlan, url: str, dest_part_id: str, base_time: Optional[Time]
) -> PostPositionReportsPlanRequest:
...
def post_position_report_plan(
    ...
    plan_req = plan_to_req(
        plan,
        pos_base_url + f"flight_position_report/{flight_id}",
        dest_participant_id,
        base_time,
    )
    with scenario.check(
...
def check_position_reports_posted_successfully(
    ...
    with scenario.check(
        ...
        logs, qt = get_position_report_logs(scenario, client, flight_id, order)
        if not logs:
            check.record_failed(
...
def get_position_report_logs(
    ...
    scenario.record_query(query)
    logs_res: PositionReportLogsResponse = ImplicitDict.parse(
        query.response.json, PositionReportLogsResponse
    )
